...
## Figure 3E (cell type single-cell resolution)
...
ggsave(filename = here("plots", dataset_name, paste0(dataset_name, "_ct_labels_sc_closeup.pdf")), width = 6, height = 6, dpi = 300)

## Figure 3F (cell type rasterization resolution)
...
for (res in res_list) {
  ...
  ggsave(file = here("plots", dataset_name, paste0(dataset_name, "_sc_with_grids_resolution_", res, ".pdf")), width = 6, height = 6, dpi = 300)
}

## Figure 3 (alpha MLE CI interval)
res_list <- list(50, 100, 200)
pairs <- c("A & B", "B & C", "B & D")
df_comb <- do.call(rbind, lapply(res_list, function(res) {
  df <- readRDS(file = here("outputs", paste0(dataset_name, "_CooccurrenceAffinity_resolution_", res, ".RDS")))
  return(data.frame(resolution = res, df))
})) %>%
  filter(pair %in% pairs)

ggplot(df_comb, aes(x = resolution, y = alpha, col = pair)) +
  geom_point() +
  geom_errorbar(data = df_comb, aes(ymin = ci.min, ymax = ci.max)) +
  theme_bw()
